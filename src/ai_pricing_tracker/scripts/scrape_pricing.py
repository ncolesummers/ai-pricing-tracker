...
class AIPricingScraper:
    def __init__(self, data_dir: Optional[Path] = None):
        """
        Initialize the scraper with a data directory.

        Args:
            data_dir: Optional directory path to store data.
                      If None, uses "data/pricing" in the current directory.
        """
        if data_dir is None:
            # Try to find the repository root or use current directory
        ...
        try:
            with sync_playwright() as p:
                ...
                try:
                    ...
                    pricing_sections = page.query_selector_all(
                        '[data-testid="pricing-card"], .pricing-card, [class*="pricing"]'
                    )
                    ...
                    for section in pricing_sections:
                        ...
                        # Look for model name
                        name_elem = section.query_selector(
                            'h2, h3, [class*="model-name"], [class*="title"]'
                        )
                        if name_elem:
                        ...
                        input_pattern = (
                            r"\$?([\d.]+)\s*(?:per|/)\s*(?:1M|million)\s*(?:input\s*)?tokens"
                        )
                        output_pattern = (
                            r"\$?([\d.]+)\s*(?:per|/)\s*(?:1M|million)\s*output\s*tokens"
                        )
                    ...
                    for script in scripts:
                        ...
                        if "pricing" in content.lower() or "price" in content.lower():
                            ...
                            try:
                                ...
                                logger.debug(f"Failed to parse JSON in script: {e}")
            ...
            with sync_playwright() as p:
                ...
                try:
                    ...
                    tables = page.query_selector_all(
                        'table, [class*="pricing-table"], [class*="price-table"]'
                    )
                    ...
                    for table in tables:
                        ...
                        for row in rows:
                            ...
                            if len(cells) >= 3:  # Model, Input price, Output price
                                ...
                                # Skip header rows
                                if (
                                    "model" in model_cell.lower()
                                    or "input" in cells[1].inner_text().lower()
                                ):
                                    continue
                                ...
                                if input_price is not None or output_price is not None:
                                    pricing_data["models"][
                                        self.normalize_model_name(model_cell)
                                    ] = {
                                        "name": model_cell,
                                        "input_price_per_1m_tokens": input_price,
                                        "output_price_per_1m_tokens": output_price,
    ...
    def save_pricing_data(self, all_pricing: Dict[str, Any]) -> None:
        ...
        timestamp_file = (
            self.data_dir / f"pricing_{datetime.utcnow().strftime('%Y%m%d_%H%M%S')}.json"
        )
    ...
    def create_simplified_pricing(self, all_pricing: Dict[str, Any]) -> Dict[str, Any]:
        ...
        simplified: Dict[str, Any] = {"last_updated": self.timestamp, "pricing": {}}
    ...
    def run(self) -> None:
        ...
        # Combine all data
        all_pricing = {"last_updated": self.timestamp, "providers": [anthropic_data, openai_data]}
